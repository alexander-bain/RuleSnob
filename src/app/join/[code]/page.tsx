...
export default function JoinPage({
  ...
  // Only look up the group AFTER the user is signed in
  useEffect(() => {
    if (!user || authLoading) return;

    async function setupAndLookup() {
      try {
  ...
  // Not signed in — show sign-in prompt (no Firestore query yet)
  if (!user) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-[#FAF8F5] to-[#F0EDE8]">
  ...
  // Signed in, waiting for group lookup
  if (!lookupDone) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-[#FAF8F5] to-[#F0EDE8]">
        <div className="h-8 w-8 animate-spin rounded-full border-2 border-[#EEEEEE] border-t-[#1B5E20]" />
  ...
  // Group not found (after authenticated lookup)
  if (!groupInfo) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-[#FAF8F5] to-[#F0EDE8]">
        <div className="mx-auto w-full max-w-[400px] px-5">
